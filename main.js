...
function getPriceWook(isbn){
	chrome.runtime.sendMessage(
		...
		text => {
			...
			var price = el.querySelectorAll("meta[property='og:url']")[0].content;
			price = price.replace("€", "").replace(",", ".");
...
function priceChecker(realPrice, site){
	...
	if(price < sitesPrice[currentSite]["price"])
		createToast();
}
...
function toggleTooltip(show){
	...
	if(show == 0){
		clearTimeout(t);
		t = setTimeout(function(){ priceTooltip.style.display = "none"; }, 500);
	} else {
...
function onMouseOver(){
	...
	toggleTooltip(1);	
	clearTimeout(t);
	t = setTimeout(function(){ toggleTooltip(0); }, 2000);
}
